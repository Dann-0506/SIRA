...
import {
  Upload, FileUp, AlertCircle, CheckCircle2, X,
  ChevronDown, ChevronUp, Info, UserMinus, Database,
} from 'lucide-react'
import axios from 'axios'
import { importarCSV } from '@/api/carga'
import { bajaMasiva } from '@/api/alumnos'
import type { CargaResultadoResponse, BajaMasivaResultado } from '@/types'
import { PageHeader } from '@/components/shared/PageHeader'
import { LoadingSpinner } from '@/components/shared/LoadingSpinner'
import { ErrorAlert } from '@/components/shared/ErrorAlert'

// ─── Tipos y definiciones de formato ─────────────────────────────────────────
...
const FORMATOS: Record<string, FormatoTipo> = {
  alumnos: {
    label: 'Alumnos',
    columnas: [
      { nombre: 'matricula',          requerida: true,  descripcion: 'Número de control único del alumno (ej. 21310001)' },
      ...
      { nombre: 'fecha_nacimiento',   requerida: true,  descripcion: 'Fecha de nacimiento en formato YYYY-MM-DD (ej. 2003-05-14)' },
      { nombre: 'clave_carrera',      requerida: true,  descripcion: 'Clave de una carrera existente en el sistema (ej. ISC)' },
      { nombre: 'curp',               requerida: false, descripcion: 'CURP (18 caracteres, dejar vacío si no disponible)' },
    ],
...
function DropZone({
  archivo, onFile, onClear,
}: { archivo: File | null; onFile: (f: File) => void; onClear: () => void }) {
    ...
    <div className="space-y-6">
      <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-6 space-y-5">
        ...
        <DropZone
          archivo={archivo}
          onFile={handleFile}
          onClear={() => { setArchivo(null); setResultado(null) }}
        />

        <div className="flex justify-end">
          <button
            onClick={() => { if (!archivo) { setGlobalError('Selecciona un archivo CSV primero.'); return }; mutation.mutate() }}
...
// ─── Sección: Bajas masivas ───────────────────────────────────────────────────

function FormatoBajas() {
  const [open, setOpen] = useState(false)
  ...
  return (
    <div className="rounded-lg border border-slate-200 overflow-hidden">
      ...
      >
        <div className="flex items-center gap-2">
          <Info className="h-4 w-4 text-blue-500" />
          Ver formato esperado para <strong>Bajas masivas</strong>
        </div>
        {open ? <ChevronUp className="h-4 w-4 text-slate-400" /> : <ChevronDown className="h-4 w-4 text-slate-400" />}
      </button>
        ...
        <div className="px-4 py-4 space-y-4 bg-white">
          <div>
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="flex items-center gap-1.5 flex-1 flex-wrap">
                <span className="text-xs px-1.5 py-0.5 rounded font-medium bg-red-100 text-red-600">Requerida</span>
                <span className="text-xs text-slate-500">Número de control del alumno a desactivar (ej. 21310001)</span>
              </div>
            </div>
          </div>
          ...
          <div>
            <p className="text-xs font-semibold text-slate-500 uppercase tracking-wide mb-2">Ejemplo de fila</p>
            <code className="block text-xs bg-slate-900 text-emerald-400 px-4 py-3 rounded-lg font-mono whitespace-pre">
              {'matricula\n21310001\n21310002\n21310003'}
            </code>
          </div>
          ...
          <div className="flex items-start gap-2 text-xs text-amber-700 bg-amber-50 border border-amber-200 rounded-lg px-3 py-2.5">
            <AlertCircle className="h-3.5 w-3.5 mt-0.5 flex-shrink-0" />
            Solo se desactivan cuentas. El historial académico (calificaciones e inscripciones) se conserva. La operación es reversible.
          </div>
...
function parseMatriculas(text: string): string[] {
  const HEADER_HINTS = ['matricula', 'num_control', 'numcontrol', 'número de control']
  return text
    .split(/\r?\n/)
    .map(l => l.trim())
    .filter(l => l.length > 0 && !HEADER_HINTS.includes(l.toLowerCase()))
}
...
function SeccionBajas() {
  ...
  const [resultado, setResultado] = useState<BajaMasivaResultado | null>(null)
  const [globalError, setGlobalError] = useState('')

  const mutation = useMutation({
    mutationFn: () => bajaMasiva(matriculas),
    onSuccess: (data) => {
      setResultado(data)
      setGlobalError('')
      if (data.desactivados.length > 0) {
        qc.invalidateQueries({ queryKey: ['alumnos'] })
      }
    },
    onError: (err) => {
      setGlobalError(axios.isAxiosError(err) ? err.response?.data?.error ?? 'Error al procesar las bajas.' : 'Error inesperado.')
      setResultado(null)
    },
  })
  ...
  const handleFile = (file: File) => {
    ...
    reader.onload = (e) => {
      ...
      const parsed = parseMatriculas(text)
  ...
  return (
    <div className="space-y-6">
      <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-6 space-y-5">
        ...
        <DropZone archivo={archivo} onFile={handleFile} onClear={handleClear} />

        {archivo && matriculas.length > 0 && (
          <div className="flex items-center gap-2 text-sm text-blue-700 bg-blue-50 border border-blue-200 rounded-lg px-4 py-3">
            <CheckCircle2 className="h-4 w-4 flex-shrink-0" />
            {matriculas.length} matrícula{matriculas.length !== 1 ? 's' : ''} detectada{matriculas.length !== 1 ? 's' : ''}
          </div>
        )}
        ...
        {archivo && matriculas.length === 0 && (
          <div className="flex items-center gap-2 text-sm text-amber-700 bg-amber-50 border border-amber-200 rounded-lg px-4 py-3">
            ...
            No se detectaron matrículas en el archivo.
          </div>
        )}

        <div className="flex justify-end">
          <button
            onClick={() => { if (!archivo || matriculas.length === 0) { setGlobalError('El archivo no contiene matrículas válidas.'); return }; mutation.mutate() }}
            disabled={mutation.isPending || !archivo || matriculas.length === 0}
          ...
          >
            {mutation.isPending ? <LoadingSpinner size="sm" /> : <UserMinus className="h-4 w-4" />}
            {mutation.isPending ? 'Procesando...' : 'Aplicar bajas'}
          </button>
        </div>
      </div>
      ...
      {resultado && (
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
          ...
          <div className="px-6 py-5 space-y-4">
            <div className="grid grid-cols-3 gap-4">
              <div className="text-center p-3 rounded-lg bg-red-50 border border-red-200">
                <p className="text-2xl font-bold text-red-700">{resultado.desactivados.length}</p>
                <p className="text-xs text-red-500 mt-0.5">Desactivados</p>
              </div>
              <div className="text-center p-3 rounded-lg bg-amber-50 border border-amber-200">
                <p className="text-2xl font-bold text-amber-700">{resultado.yaInactivos.length}</p>
                <p className="text-xs text-amber-600 mt-0.5">Ya inactivos</p>
              </div>
              <div className="text-center p-3 rounded-lg bg-slate-50 border border-slate-200">
                <p className="text-2xl font-bold text-slate-700">{resultado.noEncontrados.length}</p>
                <p className="text-xs text-slate-500 mt-0.5">No encontrados</p>
              </div>
            </div>

            {resultado.noEncontrados.length > 0 && (
              <ListaResultado
                titulo="Matrículas no encontradas"
            ...
            {resultado.yaInactivos.length > 0 && (
              <ListaResultado
                titulo="Ya estaban inactivos"
                items={resultado.yaInactivos}
                colorClass="text-amber-700"
                bgClass="bg-amber-50 border-amber-200"
              />
            )}

            {resultado.desactivados.length > 0 && resultado.noEncontrados.length === 0 && (
              <div className="flex items-center gap-2 text-sm text-emerald-700 bg-emerald-50 border border-emerald-200 rounded-lg px-4 py-3">
                <CheckCircle2 className="h-4 w-4 flex-shrink-0" />
                Todas las matrículas fueron procesadas correctamente.
              </div>
            )}
          </div>
        </div>
...
function ListaResultado({ titulo, items, colorClass, bgClass }: {
  titulo: string; items: string[]; colorClass: string; bgClass: string
}) {
  return (
    <div>
      <p className={`text-xs font-semibold mb-1.5 ${colorClass}`}>{titulo}</p>
      <div className={`border rounded-lg px-4 py-3 ${bgClass}`}>
        <p className={`text-xs font-mono ${colorClass}`}>{items.join(', ')}</p>
      </div>
    </div>
  )
}
...
type Tab = 'importar' | 'bajas'

const TABS: { id: Tab; label: string; icon: React.ReactNode }[] = [
  { id: 'importar', label: 'Importar datos', icon: <Database className="h-4 w-4" /> },
  { id: 'bajas',    label: 'Bajas masivas',  icon: <UserMinus className="h-4 w-4" /> },
]
...
export default function Herramientas() {
  ...
  return (
    <div className="max-w-2xl mx-auto">
      <PageHeader
        title="Herramientas"
        description="Operaciones masivas sobre los datos del sistema."
      />
