...
export const createMaestro = (data: { nombre: string; apellidoPaterno: string; apellidoMaterno?: string; email: string; numEmpleado: string }) =>
  client.post<MaestroResponse>(BASE, data).then(r => r.data)
export const updateMaestro = (id: number, data: { nombre: string; apellidoPaterno: string; apellidoMaterno?: string; email: string; numEmpleado: string }) =>
