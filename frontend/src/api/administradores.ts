...
export const createAdmin = (data: { nombre: string; apellidoPaterno: string; apellidoMaterno?: string; email: string; numEmpleado: string }) =>
  client.post<AdminResponse>(BASE, data).then(r => r.data)
export const updateAdmin = (id: number, data: { nombre: string; apellidoPaterno: string; apellidoMaterno?: string; email: string; numEmpleado: string }) =>
